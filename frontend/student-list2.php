...
if(isset($_POST['submit']))
{
    ...
    $name = $_POST['name'];
    $email = $_POST['email'];
    $require = $_POST['searchsts'];
    // echo $require;
    // Trim any leading/trailing whitespace
    $input = trim($input);

    //check its a valid 3 digit number
    $numbers = preg_split('/[\s,]+/', $input);
    
    $valid_numbers = [];

    // Iterate over the numbers and validate each one
...
?>
  ...
  <h3 class="blink-text" style="margin-top:50px;"> ⚠️ Kindly mark the attendance for the subject - <span><?php echo $sub ;?></span> <?php echo $sem;?>  <?php echo $sec;?> ⚠️ </h3>
...
if($count>0)
{
 ...
 <form action="<?php echo SITEURL;?>frontend/form.php"  method="POST" id="presentcontainer"  enctype="multipart/form-data"class="student-cont present" style="position:relative; margin:auto;">
  ...
  <?php
             ...
             $last_three_digit = substr($string_roll,-3);
            
    //--mark absent for searched roll number--------
            if($require=='Absent')
            {
                if (in_array($last_three_digit, $valid_numbers))
                {
                   ?>
    ...
    //---------mark present for searched roll number---------------- 
            else{
                if (in_array($last_three_digit, $valid_numbers))
                {
                   ?>
